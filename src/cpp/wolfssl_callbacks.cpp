...
int tls_recv_helper(std::unordered_map<int, std::pair<char*, int>> *accept_recv_data, server *tcp_server, char *buff, int sz, int client_socket, bool accept){
  ...
  if(recvdAmount > sz){ //too much
    ...
    (*accept_recv_data)[client_socket] = { (char*)std::malloc(recvdAmount - sz), recvdAmount - sz };
    std::memset((*accept_recv_data)[client_socket].first, 0, recvdAmount - sz);
    std::memcpy((*accept_recv_data)[client_socket].first, &all_received[sz], recvdAmount - sz);
    return sz;
