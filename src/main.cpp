...
int main(){
  ...
  // server<server_type::TLS> tcp_server(std::stoi(config_data["PORT"]), config_data["FULLCHAIN"], config_data["PKEY"], a_cb<server_type::TLS>, r_cb<server_type::TLS>, w_cb<server_type::TLS>, &basic_web_server); //pass function pointers and a custom object
  tcp_server.start();
